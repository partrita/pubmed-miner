    ...
    def test_complete_end_to_end_workflow(self):
        ...
        with patch('automated_collection.PaperCollectionService') as mock_paper_service:
            with patch('automated_collection.CitationService') as mock_citation_service:
                with patch('automated_collection.ImpactFactorService') as mock_if_service:
                    with patch('automated_collection.GitHubIssuesManager') as mock_github_manager:
                        ...
                        def mock_get_paper_details(pmids):
                            ...
                            for pmid in pmids:
                                if pmid.startswith("ml_"):
                                    papers.extend(self.create_sample_papers("machine-learning", 1))
                                elif pmid.startswith("covid_"):
                                    papers.extend(self.create_sample_papers("covid", 1))
                            return papers[:len(pmids)]
                            
                        mock_paper_instance.search_papers.side_effect = mock_search_papers
                        mock_paper_instance.get_papers_details.side_effect = mock_get_paper_details
                        
                        # Set up mock citation service
                        mock_citation_instance = Mock()
                        mock_citation_service.return_value = mock_citation_instance
                        mock_citation_instance.get_citation_count.side_effect = lambda pmid: hash(pmid) % 200
                        
                        # Set up mock impact factor service
                        mock_if_instance = Mock()
                        mock_if_service.return_value = mock_if_instance
                        mock_if_instance.get_impact_factor.side_effect = lambda journal: hash(journal) % 50 + 1
                        ...
                        mock_github_instance.create_or_update_issue.side_effect = mock_create_or_update_issue
                        ...
                        assert not any("disabled topic" in query for query in search_queries)
            ...
            with patch('automated_collection.CitationService') as mock_citation_service:
                with patch('automated_collection.ImpactFactorService') as mock_if_service:
                    with patch('automated_collection.GitHubIssuesManager') as mock_github_manager:
                        ...
                        mock_paper_instance.search_papers.side_effect = mock_search_with_failure
                        mock_paper_instance.get_papers_details.return_value = self.create_sample_papers("ml", 2)
                        ...
                        mock_citation_instance.get_citation_count.side_effect = mock_citation_with_failure
                        ...
                        assert results['topics_processed'] == 1  # Only one topic succeeded
                        assert len(results['errors']) == 1  # One topic failed
                        assert results['success'] is True  # Overall success due to partial completion
            ...
            with patch('automated_collection.CitationService') as mock_citation_service:
                with patch('automated_collection.ImpactFactorService') as mock_if_service:
                    with patch('automated_collection.GitHubIssuesManager') as mock_github_manager:
                        ...
                        mock_paper_instance.get_papers_details.return_value = self.create_sample_papers("test", 3)
                        ...
                        assert results['topics_processed'] == 2
                        assert results['papers_collected'] == 6  # 3 papers per topic, 2 topics
                        assert results['issues_created'] == 2
        ...
        try:
            with patch('automated_collection.PaperCollectionService') as mock_paper_service:
                with patch('automated_collection.GitHubIssuesManager') as mock_github_manager:
                    ...
                    assert "Starting automated essential papers collection workflow" in log_output
    ...
    def test_workflow_error_recovery(self):
        ...
        with patch('automated_collection.PaperCollectionService') as mock_paper_service:
            with patch('automated_collection.GitHubIssuesManager') as mock_github_manager:
                ...
                mock_paper_instance.search_papers.side_effect = mock_search_mixed_results
                mock_paper_instance.get_papers_details.return_value = self.create_sample_papers("test", 2)
                ...
                assert results['issues_created'] == 1  # Issue created for successful topic
            ...
            with patch('automated_collection.CitationService') as mock_citation_service:
                with patch('automated_collection.ImpactFactorService') as mock_if_service:
                    with patch('automated_collection.GitHubIssuesManager') as mock_github_manager:
                        ...
                        # Return many PMIDs
                        mock_paper_instance.search_papers.return_value = [str(i) for i in range(100)]
                        mock_paper_instance.get_papers_details.return_value = self.create_sample_papers("perf", 100)
                        ...
                        assert results['success'] is True
                        assert results['papers_collected'] == 200  # 100 papers per topic, 2 topics