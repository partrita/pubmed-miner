...
from src.pubmed_miner.utils.error_handler import GitHubError, APIError
...
class TestGitHubIssuesManager:
    ...
    def test_sanitize_topic_name(self):
        ...
        assert self.manager._sanitize_topic_name("Machine Learning") == "machine-learning"
        assert self.manager._sanitize_topic_name("COVID-19 Research") == "covid-19-research"
    ...
    def test_get_statistics(self):
        ...
        required_keys = [
            'issues_created', 'issues_updated', 'comments_added',
            'api_calls', 'error_count', 'last_activity'
        ]
    ...
    def test_close_issue(self):
        ...
        with patch('src.pubmed_miner.services.github_manager.requests.patch') as mock_patch:
            mock_response = Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = {
                'number': 42,
            ...
            try:
                with patch.object(self.manager, '_find_existing_issue') as mock_find:
                    ...
                    with patch.object(self.manager, '_create_issue') as mock_create:
                        ...
                        result = self.manager.create_or_update_issue(topic, self.sample_papers)
    ...
    def test_mock_mode_create_issue(self):
        """Test creating issue in mock mode."""
        mock_config = GitHubConfig(
            token="mock_token_for_local_testing",
            repository="test/repo",
            issue_labels=["test"]
        ...
        result = mock_manager.create_or_update_issue("test-topic", self.sample_papers)
    ...
    def test_mock_mode_find_existing_issue(self):
        ...
        # Mock mode always returns None (no existing issues)
        result = mock_manager.find_existing_issue("test-topic")
