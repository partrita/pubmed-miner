...
class TestScoringEngine:
    ...
    def test_calculate_relevance_score(self):
        ...
        relevance_score = self.engine._calculate_relevance_score(
            self.sample_paper, query
        )
        assert relevance_score > 50  # Should have good relevance

        # Test with non-matching paper
        non_matching_score = self.engine._calculate_relevance_score(
            self.old_paper, query
        )
        ...
        empty_query_score = self.engine._calculate_relevance_score(
            self.sample_paper, ""
        )
    ...
    def test_calculate_paper_score_components(self):
        ...
        recency_score = self.engine._calculate_recency_score(
            self.sample_paper.publication_date
        )
    ...
    def test_normalize_journal_name(self):
        ...
        assert (
            self.engine._normalize_journal_name("Nature Medicine") == "nature medicine"
        )

        # Test with prefixes/suffixes
        assert (
            self.engine._normalize_journal_name("The Journal of Medicine (Online)")
            == "journal of medicine"
        )

        # Test with extra spaces
        assert (
            self.engine._normalize_journal_name("  Nature   Genetics  ")
            == "nature genetics"
        )
