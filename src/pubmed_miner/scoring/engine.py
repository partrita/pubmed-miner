...
class ScoringEngine:
    ...
    def calculate_paper_score(
        ...
        relevance_score = (
            self._calculate_relevance_score(paper, query) if query else 50.0
        )
        ...
        logger.debug(
            f"Score for {paper.pmid}: C:{citation_score:.1f} IF:{impact_score:.1f} "
    ...
    def select_essential_papers(
        ...
        logger.info(
            f"Selected {len(essential_papers)} essential papers from {len(papers)} total"
        )
    ...
    def score_paper_batch(
        ...
        for paper in papers:
            try:
                # Get citation count (handle None values safely)
                raw_citation_count = citations.get(paper.pmid)
                citation_count = self._safe_int_value(raw_citation_count, default=0)
                ...
                # Ensure impact factor is valid
                journal_if = self._safe_float_value(journal_if, default=0.0)
                ...
                logger.warning(f"Failed to score paper {paper.pmid}: {e}")
                continue
    ...
    def _calculate_citation_score(self, citations: Optional[int]) -> float:
        ...
        Args:
            citations: Citation count (can be None)

        Returns:
            Normalized citation score
        """
        # Safely handle None and negative values
        ...
        if safe_citations >= max_citations:
            return 100.0

        # Logarithmic scaling: log(citations + 1) / log(max_citations + 1) * 100
        score = (math.log(safe_citations + 1) / math.log(max_citations + 1)) * 100
    ...
    def _calculate_impact_factor_score(self, impact_factor: float) -> float:
        ...
        """
        # Safely handle None and negative values
        safe_if = self._safe_float_value(impact_factor, default=0.0)
        ...
        # Logarithmic scaling
        score = (math.log(safe_if + 1) / math.log(max_if + 1)) * 100
        return min(100.0, score)
    ...
    def _calculate_relevance_score(self, paper: Paper, query: Optional[str]) -> float:
        ...
        abstract_score = (
            (abstract_matches / len(query_terms)) * 100 if query_terms else 0
        )
    ...
    def get_score_breakdown(
        ...
        relevance_score = (
            self._calculate_relevance_score(scored_paper, query) if query else 50.0
        )
    ...
    def get_scoring_statistics(
        ...
        citations = [
            p.citation_count for p in scored_papers if p.citation_count is not None
        ]
