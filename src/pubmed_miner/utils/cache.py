...
class CacheManager:
    ...
    def _init_database(self) -> None:
        ...
        with self._get_connection() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_citations_updated ON citations(last_updated)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_impact_factors_updated ON impact_factors(last_updated)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_metadata_updated ON paper_metadata(last_updated)")
    ...
    def clear_expired_citations(self, max_age_days: int = 7) -> int:
        ...
        try:
            ...
            with self._get_connection() as conn:
                cursor = conn.execute(
                    "DELETE FROM citations WHERE last_updated < ?",
                    (cutoff_date,)
                )
    ...
    def save_impact_factor(self, impact_factor: ImpactFactorCache) -> None:
        ...
        try:
            ...
            logger.error(f"Error saving impact factor cache for {impact_factor.journal_name}: {e}")
            
    def search_similar_journals(self, journal_name: str, limit: int = 5) -> List[ImpactFactorCache]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    results.append(ImpactFactorCache(
                        journal_name=row['journal_name'],
                        impact_factor=row['impact_factor'],
                        year=row['year'],
                        last_updated=row['last_updated'],
    ...
    def get_paper_metadata(self, pmid: str) -> Optional[PaperMetadataCache]:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.execute(
                    "SELECT * FROM paper_metadata WHERE pmid = ?",
                    (pmid,)
                )
                row = cursor.fetchone()
    ...
    def cleanup_cache(self, citation_max_age: int = 7, impact_factor_max_age: int = 365) -> Dict[str, int]:
        ...
        try:
            ...
            with self._get_connection() as conn:
                cursor = conn.execute(
                    "DELETE FROM impact_factors WHERE last_updated < ?",
                    (cutoff_date,)
                )
    ...
    def export_cache_data(self, output_file: str, table_name: Optional[str] = None) -> None:
        ...
        try:
            ...
            with self._get_connection() as conn:
                tables = [table_name] if table_name else ['citations', 'impact_factors', 'paper_metadata']
    ...
    def import_cache_data(self, input_file: str, overwrite: bool = False) -> Dict[str, int]:
            ...
            with self._get_connection() as conn:
                for table_name, rows in import_data.items():
                    ...
                    for row in rows:
                        ...
                        operation = "INSERT OR REPLACE" if overwrite else "INSERT OR IGNORE"
                        ...
                        if table_name == 'citations':
                            conn.execute(f"""
                                ...
                                (pmid, citation_count, last_updated, source)
                                VALUES (?, ?, ?, ?)
                            """, (row['pmid'], row['citation_count'], row['last_updated'], row['source']))
                            ...
                            conn.execute(f"""
                                ...
                                (journal_name, impact_factor, year, last_updated, source)
                                VALUES (?, ?, ?, ?, ?)
                            """, (row['journal_name'], row['impact_factor'], row['year'], 
                                 row['last_updated'], row['source']))
                            ...
                            conn.execute(f"""
                                ...
                                (pmid, title, authors_json, journal, publication_date, abstract, doi, last_updated)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                            """, (row['pmid'], row['title'], row['authors_json'], row['journal'],
                                 row['publication_date'], row['abstract'], row['doi'], row['last_updated']))
