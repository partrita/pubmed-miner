...
class ConfigurationManager:
    ...
    def get_github_settings(self) -> GitHubConfig:
        ...
        if not token:
            logger.warning("GitHub token not found - GitHub features will be disabled for local testing")
            token = 'mock_token_for_local_testing'
            
        return GitHubConfig(
            token=token,
            repository=github_data.get('repository', 'local/test-repo'),
            issue_labels=github_data.get('issue_labels', ['essential-papers', 'automated'])
        )
    ...
    def create_default_configs(self) -> None:
        ...
        if not self.topics_file.exists():
            ...
            with open(self.topics_file, 'w', encoding='utf-8') as f:
                yaml.dump(default_topics, f, default_flow_style=False, allow_unicode=True)
    ...
    def _load_cache_settings(self) -> Dict[str, Any]:
        ...
        """
        settings = self._load_settings()
        return settings.get('cache_settings', {
            'citation_cache_days': 7,
            'impact_factor_cache_days': 365,
            'paper_metadata_cache_days': 30
        })
