...
class ErrorHandler:
    ...
    def handle_api_error(self, error: APIError, context: str = "", severity: ErrorSeverity = ErrorSeverity.MEDIUM) -> None:
        ...
        logger.log(log_level, f"API Error{' in ' + context if context else ''}: {error}")
    ...
    def handle_data_error(self, error: DataError, context: str = "", severity: ErrorSeverity = ErrorSeverity.MEDIUM) -> None:
        ...
        log_level = self._get_log_level(severity)
        logger.log(log_level, f"Data Error{' in ' + context if context else ''}: {error}")
    ...
    def handle_configuration_error(self, error: ConfigurationError, context: str = "") -> None:
        ...
        logger.critical(f"Configuration Error{' in ' + context if context else ''}: {error}")
    ...
    def _handle_validation_error(self, error: ValidationError, context: str) -> None:
        ...
        if error.field and error.value is not None:
            logger.error(f"Validation failed for field '{error.field}' with value '{error.value}': {error}")
        else:
                    ...
                    try:
                        ...
                        if attempt < max_attempts - 1:  # Don't sleep on last attempt
                            ...
                            if isinstance(e, RateLimitError) and e.retry_after:
                                ...
                                logger.info(f"Rate limited, waiting {sleep_time} seconds as requested...")
                            else:
    ...
    def circuit_breaker(
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args, **kwargs) -> Any:
                ...
                if (state['is_open'] and state['last_failure'] and 
                    (now - state['last_failure']).seconds >= recovery_timeout):
                ...
                if state['is_open']:
                    raise expected_exception(f"Circuit breaker open for {func.__name__}")
    ...
    def safe_execute(
        ...
        try:
            ...
            if log_errors:
                ...
                logger.error(f"Error in {func_name}{' (' + context + ')' if context else ''}: {e}")
        ...
        for i, item in enumerate(items):
            try:
                ...
                if max_failures and len(errors) >= max_failures:
                    logger.error(f"Stopping batch processing after {len(errors)} failures")
                    break
...
def handle_exceptions(default_return: Any = None, log_errors: bool = True, context: str = ""):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                execution_time = time.time() - start_time
                log.warning(f"{func.__name__} failed after {execution_time:.2f} seconds: {e}")
                raise
        return wrapper
    return decorator