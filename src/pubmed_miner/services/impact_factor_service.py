...
class ImpactFactorService:
    ...
    def match_journal_name(self, partial_name: str) -> Optional[str]:
        ...
        if self.cache_manager and best_score < 0.8:
            cached_matches = self.cache_manager.search_similar_journals(normalized_partial)
            for cached_journal in cached_matches:
                score = SequenceMatcher(None, normalized_partial, 
                                      self._normalize_journal_name(cached_journal.journal_name)).ratio()
                if score > best_score:
                    best_score = score
    ...
    def _get_builtin_impact_factor(self, normalized_name: str) -> Optional[float]:
        ...
        for known_journal, impact_factor in self.known_impact_factors.items():
            ...
            if similarity > 0.9:  # Very high similarity threshold for built-in data
                logger.debug(f"Fuzzy match: {normalized_name} -> {known_journal} (similarity: {similarity:.2f})")
                return impact_factor
    ...
    def _estimate_by_type(self, normalized_name: str) -> float:
        ...
        if any(term in normalized_name for term in ['medicine', 'medical', 'clinical', 'therapy']):
            return 6.0
            
        # Biology and life sciences
        if any(term in normalized_name for term in ['biology', 'biological', 'life', 'molecular']):
            return 4.5
            
        # Computer science and bioinformatics
        if any(term in normalized_name for term in ['bioinformatics', 'computational', 'computer']):
            return 3.5
    ...
    def _get_cached_impact_factor(self, journal_name: str) -> Optional[float]:
        ...
        try:
            ...
            if cache_entry and not cache_entry.is_expired():
                logger.debug(f"Using cached impact factor for {journal_name}: {cache_entry.impact_factor}")
    ...
    def _cache_impact_factor(self, journal_name: str, impact_factor: float, source: str) -> None:
        """Cache impact factor.
        ...
        try:
            ...
            logger.debug(f"Cached impact factor for {journal_name}: {impact_factor} from {source}")
        except Exception as e:
    ...
    def get_journal_statistics(self) -> Dict[str, any]:
        ...
        if self.known_impact_factors:
            stats['average_impact_factor'] = sum(self.known_impact_factors.values()) / len(self.known_impact_factors)
            
            # Get top 5 journals by impact factor
            sorted_journals = sorted(
                self.known_impact_factors.items(),
                key=lambda x: x[1],
                reverse=True
            )
