...
from ..utils.error_handler import APIError, RateLimitError, retry_api_calls
...
class CitationService:
    ...
    def batch_get_citations(self, pmids: List[str]) -> Dict[str, int]:
        ...
        Returns:
            Dictionary mapping PMID to citation count
        """
        citations = {}
        ...
        for i in range(0, len(uncached_pmids), batch_size):
            ...
            batch_citations = self._fetch_citation_batch(batch)
    ...
    def _fetch_citation_batch(self, pmids: List[str]) -> Dict[str, int]:
    ...
    def _get_cached_citation(self, pmid: str) -> Optional[int]:
        ...
        try:
            ...
            if cache_entry and not cache_entry.is_expired():
                logger.debug(f"Using cached citation for {pmid}: {cache_entry.citation_count}")
