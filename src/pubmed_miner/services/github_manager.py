...
class GitHubIssuesManager:
    ...
    def __init__(self, config: GitHubConfig):
        ...
        self.mock_mode = (config.token == 'mock_token_for_local_testing' or 
                         not config.token or 
                         config.token.startswith('mock_'))
        
        if self.mock_mode:
            logger.info(f"GitHubIssuesManager initialized in MOCK MODE for local testing")
            self.headers = {
    ...
    def create_or_update_issue(self, topic: str, papers: List[ScoredPaper]) -> Dict[str, Any]:
        ...
        if existing_issue:
            logger.info(f"Updating existing issue #{existing_issue['number']} for topic: {topic}")
    ...
    def find_existing_issue(self, topic: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            search_query = f"repo:{self.config.repository} is:issue \"[Essential Papers] {topic}\""
            ...
            if issues:
                ...
                issue = issues[0]
                logger.debug(f"Found existing issue #{issue['number']} for topic: {topic}")
                return issue
    ...
    def format_paper_list(self, papers: List[ScoredPaper]) -> str:
        ...
        for paper in sorted_papers:
            ...
            if paper.doi:
                paper_lines.insert(-1, f"**DOI:** [{paper.doi}](https://doi.org/{paper.doi})")
    ...
    @retry_api_calls(max_attempts=3, delay=1.0)
    def _update_issue(self, issue: Dict[str, Any], papers: List[ScoredPaper]) -> Dict[str, Any]:
    ...
    def add_comment_for_changes(self, issue: Dict[str, Any], changes: List[str]) -> None:
        ...
        comment_body += f"**Updated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}\n\n"
    ...
    def list_essential_paper_issues(self) -> List[Dict[str, Any]]:
        ...
        """
        try:
            search_query = f"repo:{self.config.repository} is:issue \"[Essential Papers]\""
            url = f"{self.base_url}/search/issues"
    ...
    def validate_access(self) -> bool:
        ...
        try:
            ...
            can_create_issues = permissions.get("push", False) or permissions.get("admin", False)
    ...
    def detect_paper_changes(
        self, 
        old_papers: List[ScoredPaper], 
        new_papers: List[ScoredPaper]
    ) -> List[str]:
        ...
        for pmid in set(old_dict.keys()) & set(new_dict.keys()):
            ...
            if old_rank != new_rank:
                ...
                rank_changes.append(f"  {direction} {paper.title}: #{old_rank} → #{new_rank}")
    ...
    def _mock_create_or_update_issue(self, topic: str, papers: List[ScoredPaper]) -> Dict[str, Any]:
        ...
        if papers:
            logger.info(f"[MOCK] Top paper: {papers[0].title} (Score: {papers[0].score:.1f})")
    ...
    def _mock_create_or_update_issue(self, topic: str, papers: List[ScoredPaper]) -> Dict[str, Any]:
        ...
        for i, paper in enumerate(papers[:3]):  # Show first 3 papers
            logger.info(f"[MOCK MODE] Paper {i+1}: {paper.title} (Score: {paper.score:.1f})")
