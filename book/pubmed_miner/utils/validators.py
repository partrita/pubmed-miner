    ...
    def validate_topic_config(topic_data: Dict[str, Any]) -> List[str]:
        ...
        if "name" in topic_data:
            ...
            if not re.match(r"^[a-zA-Z0-9_-]+$", name):
                errors.append(
                    f"Topic name '{name}' contains invalid characters. Use only letters, numbers, hyphens, and underscores."
                )

        # Validate numeric fields
        numeric_fields = {"max_papers": (1, 10000), "essential_count": (1, 100)}
                ...
                if not authors:
                    ...
                    for i, author in enumerate(authors):
                        if not isinstance(author, str) or not author.strip():
                            errors.append(
                                f"Author at index {i} must be a non-empty string"
                            )
...
def sanitize_string(
    ...
    if allowed_chars and not re.match(allowed_chars, value):
        raise ValidationError("String contains invalid characters", value=value)
        ...
        try:
            ...
            if errors:
                results["invalid_items"].append(
                    {"index": i, "item": item, "errors": errors}
                )
                ...
                if len(results["errors"]) >= max_errors:
                    results["errors"].append(
                        f"... and more (stopped at {max_errors} errors)"
                    )
                    break
            else:
            ...
            results["invalid_items"].append(
                {"index": i, "item": item, "errors": [f"Validation exception: {e}"]}
            )
            results["error_count"] += 1

    results["success_rate"] = (
        len(results["valid_items"]) / results["total_items"]
