    ...
    for entry in entries:
        try:
            title_match = re.search(
                r'<div class="paper-title">\d+\. (.+?)</div>', entry
            )
            journal_match = re.search(r"저널: (.+?)<br>", entry) or re.search(
                r"저널: (.+?)\n", entry
            )
            ...
            if title_match and pmid_match:
                paper = ScoredPaper(
                    ...
                    authors=[
                        "Unknown"
                    ],  # Authors list cannot be empty per model validation
                    journal=(
                        journal_match.group(1).strip() if journal_match else "Unknown"
                    ),
                    publication_date=(
...
def main():
    ...
    daily_files = sorted(book_src.glob("202[4-9]/[0-1][0-9]/[0-3][0-9]_*.md"))

    logger.info(f"Found {len(daily_files)} daily files to consolidate")

    consolidated_count = 0
    for daily_file in daily_files:
        papers, topic, _ = parse_daily_page(daily_file)
        if papers:
            paper_date = papers[0].publication_date
            relative_path = mdbook_manager.update_monthly_page(
                topic, papers, date=paper_date
            )
    ...
    print(
        "2. You can manually delete the old daily directories (e.g., book_src/2026/01/) if everything looks good."
    )
